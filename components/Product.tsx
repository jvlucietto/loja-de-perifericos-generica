...
function Product({ title, price, img, discount, popularRank }: Props) {
  ...
  return (
    <div  onMouseOver={mouseOver} onMouseLeave={mouseOut} className='  lg:block h-fit  w-fit bg-white rounded border-1 border-neutral-200 hover:shadow-3xs cursor-pointer cart-invisible  '>
      ...
      <div className=' relative  px-10 flex justify-center items-center '>
        ...
        {
          popularRank && (<div className=' m-2 lg:m-0 absolute bottom-0 left-0 z-30 bg-orange-500 text-xs text-white font-semibold p-1 rounded'>{popularRank}º mais vendido</div>)
        }
      </div>
      <div className='  flex flex-col items-start justify-center lg:m-2 font-medium lg:w-64  '>
        <div className='  custom-elips m-2 lg:m-0 flex  h-[54px] w-fit   overflow-hidden text-ellipsis   '>
          {title}
        </div>

        <div className= ' m-2 lg:m-0  flex flex-col items-start justify-center w-fit h-[67px] '>
